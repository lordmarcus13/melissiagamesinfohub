
# ✦ MELISSIA GAMES NEW PLAYER REWARDS ✦

Welcome to Melissia Games! Here are some rewards to help you get started:

-  [Click Here to Register/Create Account](https://office.melissia.games/register)

-  [Click Here to Download Game](https://melissiagamesinfohub.vercel.app/en/wiki/download-setup)

## ✧ COUPON REWARDS ✧


-  Peridot Leaf

```LEAFS-FOR-THE-NOOBS```

-  530% Exp Scrolls

```EXP-FOR-THE-NOOBS```

-  2500 Pearls

```PEARLS-FOR-THE-NOOBS```

-  New Starter Box

```THINGIES-FOR-THE-NOOBS```

-  New Starter Elixirs

```ELIXIRS-FOR-THE-NOOBS```

-  New Starter Perfumes

```PERFUMES-FOR-THE-NOOBS```

-  New Starter Enhancing

```ENHANCING-FOR-THE-NOOBS```

-  Garmoth Scales x 30

```NEW-PLAYER-GARMOTH```

-  Cron Stones x 7.500

```NEW-PLAYER-CRONS```

-  Advice of Valks (+50) x 20

```NEW-PLAYER-VALKS```

-  Melissia Premium 1 day x 7

```NEW-PLAYER-PREMIUM```

-  Crystal Restoration Coupon x 3

```NEW-PLAYER-CRYSTAL-RESTORE```

-  Golden Hedgehogs (Pearl) x 5

```NEW-PLAYER-HEDGEHOG```

-  Arctic Fox x 5

```NEW-PLAYER-FOX```

-  Transaction Maids x 10

```NEW-PLAYER-TMAID```

-  Storage Maids x 10

```NEW-PLAYER-SMAID```

> **Important Note:** Some rewards may appear as a single item in the inventory even when you have received multiple units. This is a visual bug in the game. You actually receive the specified number of items, but they are stacked in one inventory slot. For example, if you receive 7 x 1-day premium, you will only see one item in your inventory. However, there are 7 items, but they are in the same slot. When you use it by right-clicking, the total amount will be correctly deducted. 

> You can also verify this by moving the items to storage.

> Coupons are also available on [Discord](https://discord.com/channels/906712777333288990/1421996185400377374/1501962560470257704)

> If you do not know how to redeem coupons, [click here](https://melissiagamesinfohub.vercel.app/en/wiki/active-coupons)


## ✧ IN-GAME REWARD ✧

- 2 Pets (Dog & Cat)
- +8 Inventory Expansion Slots & +150 LT Weight
- Tier 7 Horse
- Temp (15 Days): +15 Kzarka/Accuracy/Awakening Weapon Boxes, Karlstein Classic Box, Value Pack 
- Temp (20 Days): Blessing of Kamasylve, Secret Book of Old Moon
- Melissia Premium (1 Day)
![New Player Rewards](https://i.imgur.com/JXkwbbt.png)

## ✧ NEWBIE BUFF ✧

New players receive the **Newbie Buff for 30 days** (+20k LT, +2.5 FS). *Returning veterans (90+ days absent) get it for 14 days.*
Claim via the Challenges menu (Y button).